...
namespace pfd
{
	...
	namespace internal
	{
		...
		{
			int len =
			    WideCharToMultiByte(CP_UTF8, 0, str.c_str(), (int)str.size(), nullptr, 0, nullptr, nullptr);
	...
	inline void settings::verbose(bool value) { settings().flags(flag::is_verbose) = value; }
...
#elif __EMSCRIPTEN__
	inline void internal::executor::start(int exit_code) { m_exit_code = exit_code; }
	...
	{
		std::string res = std::regex_replace(str.c_str(), std::regex("['\"]"), "$&$&").c_str();
		return "'" + res + "'";
	}
						  ...
						  opt options /* = opt::none */)
	{
		...
		for (size_t i = 0; i + 1 < filters.size(); i += 2)
		{
			...
			std::string regexFilter = filters[i + 1].c_str();
			std::string res = std::regex_replace(regexFilter, whitespace, ";").c_str();
			filter_list += res + '\0';
		}
		filter_list += '\0';
		...
		m_async->start_func([this, in_type, title, default_path, filter_list,
			...
			if (in_type == type::save)
			{
				...
				dll::proc<BOOL WINAPI(LPOPENFILENAMEW)> get_save_file_name(comdlg32,
											   "GetSaveFileNameW");
	...
	inline std::string internal::file_dialog::select_folder_vista(IFileDialog* ifd, bool force_path)
	{
		...
		dll::proc<HRESULT WINAPI(PCWSTR, IBindCtx*, REFIID, void**)> create_item(shell32,
	...
	inline notify::notify(std::string const& title, std::string const& message, icon _icon /* = icon::info */)
	{
		...
		// - NIF_INFO    Use a balloon ToolTip instead of a standard ToolTip. The szInfo, uTimeout, szInfoTitle,
		// and dwInfoFlags members are valid.
		// - NIF_GUID    Reserved.
