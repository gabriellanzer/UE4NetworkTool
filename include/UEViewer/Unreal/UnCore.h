...
struct CGameFileInfo
{
  ...
  public:
	...
	const char* GetExtension() const { return ShortFilename + ExtensionOffset; }
	...
	const FString& GetPath() const { return GetPathByIndex(FolderIndex); }
	...
	FORCEINLINE bool IsPackage() const { return (Flags & GFI_Package) != 0; }

	FORCEINLINE bool IsIOStoreFile() const { return (Flags & GFI_IOStoreFile) != 0; }
...
class FName
{
  public:
	...
	inline bool operator==(const char* String) const { return (stricmp(Str, String) == 0); }

	FORCEINLINE bool operator!=(const FName& Other) const { return !operator==(Other); }

	FORCEINLINE bool operator!=(const char* String) const { return !operator==(String); }

	FORCEINLINE const char* operator*() const { return Str; }
	FORCEINLINE operator const char*() const { return Str; }
};
...
class FArchive
{
  ...
  public:
	...
	FArchive()
		: ArPos(0), ArStopper(0), ArVer(100000) //?? something large
		  ,
		  ArLicenseeVer(0), IsLoading(true), ReverseBytes(false), Game(GAME_UNKNOWN), Platform(PLATFORM_PC)
	{
	}
	...
	inline int Engine() const { return (Game & GAME_ENGINE); }

	virtual bool IsCompressed() const { return false; }

#if UNREAL4
	virtual bool ContainsEditorData() const { return false; }
#endif
	...
	virtual int Tell() const { return ArPos; }

	virtual int GetFileSize() const { return 0; }
	...
	virtual int64 Tell64() const { return Tell(); }

	virtual int64 GetFileSize64() const { return GetFileSize(); }
	...
	virtual void SetStopper(int Pos) { ArStopper = Pos; }

	virtual int GetStopper() const { return ArStopper; }
	...
	virtual FArchive& operator<<(UObject*& /*Obj*/) { return *this; }
	...
	static const char* StaticGetName() { return "FArchive"; }

	virtual const char* GetName() const { return StaticGetName(); }

	virtual bool IsA(const char* type) const { return !strcmp(type, "FArchive"); }
...
class FPrintfArchive : public FArchive
{
  ...
  public:
	...
	virtual void Seek(int Pos) { appError("FPrintfArchive::Seek"); }

	virtual void Serialize(void* data, int size) { appPrintf("%s", data); }
};
...
class FFileArchive : public FArchive
{
  ...
  public:
	...
	const char* GetFileName() const { return FullName; }
...
class FReaderWrapper : public FArchive
{
  ...
  public:
	...
	FReaderWrapper(FArchive* File, int Offset = 0) : Reader(File), ArPosOffset(Offset) {}
	virtual ~FReaderWrapper() { delete Reader; }
	virtual bool IsCompressed() const { return Reader->IsCompressed(); }
	virtual void Seek(int Pos) { Reader->Seek(Pos + ArPosOffset); }
	virtual int Tell() const { return Reader->Tell() - ArPosOffset; }
	virtual int GetFileSize() const { return Reader->GetFileSize() - ArPosOffset; }
	virtual void Serialize(void* data, int size) { Reader->Serialize(data, size); }
	virtual void SetStopper(int Pos) { Reader->SetStopper(Pos + ArPosOffset); }
	virtual int GetStopper() const { return Reader->GetStopper() - ArPosOffset; }
	virtual bool IsOpen() const { return Reader->IsOpen(); }
...
class FMemReader : public FArchive
{
  ...
  public:
	FMemReader(const void* data, int size) : DataPtr((const byte*)data), DataSize(size)
	{
	...
	virtual bool IsEof() const { return ArPos >= DataSize; }
	...
	virtual void Serialize(void* data, int size)
	{
		...
		if (ArPos + size > DataSize) appError("Serializing behind end of buffer");
	...
	virtual int GetFileSize() const { return DataSize; }
...
class FMemWriter : public FArchive
{
  ...
  public:
	...
	const TArray<byte>& GetData() const { return *Data; }
...
#define DROP_REMAINING_DATA(Ar) Ar.Seek(Ar.GetStopper());
...
struct FVector4
{
	...
	friend FArchive& operator<<(FArchive& Ar, FVector4& V) { return Ar << V.X << V.Y << V.Z << V.W; }
};
...
struct FQuat
{
	...
	friend FArchive& operator<<(FArchive& Ar, FQuat& F) { return Ar << F.X << F.Y << F.Z << F.W; }
};
...
struct FCoords
{
	...
	friend FArchive& operator<<(FArchive& Ar, FCoords& F) { return Ar << F.Origin << F.XAxis << F.YAxis << F.ZAxis; }
};
...
struct FBox
{
	...
	{
#if UC2
		if (Ar.Engine() == GAME_UE2X && Ar.ArVer >= 146) return Ar << Box.Min << Box.Max;
#endif // UC2
...
struct FPlane : public FVector
{
	...
	friend FArchive& operator<<(FArchive& Ar, FPlane& S) { return Ar << (FVector&)S << S.W; };
};
...
struct FMatrix
{
	...
	friend FArchive& operator<<(FArchive& Ar, FMatrix& F) { return Ar << F.XPlane << F.YPlane << F.ZPlane << F.WPlane; }
};
...
class FScale
{
  public:
	...
	// Serializer.
	friend FArchive& operator<<(FArchive& Ar, FScale& S) { return Ar << S.Scale << S.SheerRate << S.SheerAxis; }
};
...
struct FLinearColor
{
	...
	friend FArchive& operator<<(FArchive& Ar, FLinearColor& C) { return Ar << C.R << C.G << C.B << C.A; }
};
...
struct FIntPoint
{
	...
	friend FArchive& operator<<(FArchive& Ar, FIntPoint& V) { return Ar << V.X << V.Y; }
};
...
struct FIntVector
{
	...
	friend FArchive& operator<<(FArchive& Ar, FIntVector& V) { return Ar << V.X << V.Y << V.Z; }
};
...
struct FVector2D
{
	...
	friend FArchive& operator<<(FArchive& Ar, FVector2D& V) { return Ar << V.X << V.Y; }
};
...
struct FTransform
{
	...
	friend FArchive& operator<<(FArchive& Ar, FTransform& T) { return Ar << T.Rotation << T.Translation << T.Scale3D; }
};
...
struct TTypeInfo
{
	...
	{
		IsSimpleType = 0
	}; // type consists of NumFields fields of integral type, sizeof(type) == FieldSize
	enum
	{
		IsRawType = 0
	}; // type's on-disk layout exactly matches in-memory layout
	enum
	{
		IsPod = IS_POD(T)
	}; // type has no constructor/destructor
};
...
#define SIMPLE_TYPE(Type, BaseType)                                                                                    \
	...
	{                                                                                                                  \
		enum                                                                                                           \
		{                                                                                                              \
			FieldSize = sizeof(BaseType)                                                                               \
		};                                                                                                             \
		enum                                                                                                           \
		{                                                                                                              \
			NumFields = sizeof(Type) / sizeof(BaseType)                                                                \
		};                                                                                                             \
		enum                                                                                                           \
		{                                                                                                              \
			IsSimpleType = 1                                                                                           \
...
class FArray
{
	...
	template <int N>
	friend class FStaticString;

  public:
	FORCEINLINE FArray() : DataCount(0), MaxCount(0), DataPtr(NULL) {}
	~FArray();

	void MoveData(FArray& Other, int elementSize);

	FORCEINLINE void* GetData() { return DataPtr; }
	FORCEINLINE const void* GetData() const { return DataPtr; }
	FORCEINLINE int Num() const { return DataCount; }
	FORCEINLINE int Max() const { return MaxCount; }
  ...
  protected:
	...
	FORCEINLINE bool IsStatic() const { return DataPtr == (void*)(this + 1); }
...
class TArray : public FArray
{
  ...
  public:
	...
	FORCEINLINE T* GetData() { return (T*)DataPtr; }
	FORCEINLINE const T* GetData() const { return (const T*)DataPtr; }
	FORCEINLINE unsigned int GetTypeSize() const { return sizeof(T); }
#if !DO_ASSERT
	// version without verifications, very compact
	FORCEINLINE T& operator[](int index) { return *((T*)DataPtr + index); }
	FORCEINLINE const T& operator[](int index) const { return *((T*)DataPtr + index); }
#elif DO_GUARD_MAX
	...
	FORCEINLINE T& operator[](int index) { return *(T*)GetItem(index, sizeof(T)); }
	FORCEINLINE const T& operator[](int index) const { return *(T*)GetItem(index, sizeof(T)); }
#endif // DO_ASSERT && !DO_GUARD_MAX

	FORCEINLINE T& Last(int IndexFromEnd = 0) { return *(T*)GetItem(DataCount - IndexFromEnd - 1, sizeof(T)); }
	...
	FORCEINLINE void InsertZeroed(int index, int count = 1) { FArray::InsertZeroed(index, count, sizeof(T)); }
	...
	{
		if (count > MaxCount) ResizeTo(count);
	}
	...
	FORCEINLINE void Sort(int (*cmpFunc)(const T*, const T*)) { QSort<T>((T*)DataPtr, DataCount, cmpFunc); }
	...
	{
		if (!TTypeInfo<T>::IsPod && Ar.IsLoading) Destruct(0, Num());
  ...
  protected:
	...
	TArray& operator=(const TArray& Other) { return *this; }
	// but allow rvalue copying - for FString
	TArray(TArray&& Other) { MoveData(Other, sizeof(T)); }
	...
	static FArchive& SerializeArray(FArchive& Ar, void* Array) { return Ar << *(TArray<T>*)Array; }
	// fast version of operator[] without assertions (may be used in safe code)
	FORCEINLINE T& Item(int index) { return *((T*)DataPtr + index); }
	FORCEINLINE const T& Item(int index) const { return *((T*)DataPtr + index); }
...
FArchive& operator<<(FArchive& Ar, TArray<T>& A)
{
	...
	if (TTypeInfo<T>::IsRawType) return A.SerializeRaw(Ar, TArray<T>::SerializeItem, sizeof(T));
...
FORCEINLINE void* operator new(size_t size, TArray<T>& Array)
{
	if (bCheck)
	{
		...
		if (size != sizeof(T)) appError("TArray::operator new: size mismatch");
	}
...
struct TMapPair
{
	...
	friend FArchive& operator<<(FArchive& Ar, TMapPair& V) { return Ar << V.Key << V.Value; }
};
...
class TMap : public TArray<TMapPair<TK, TV>>
{
  public:
	...
	FORCEINLINE const TV* Find(const TK& Key) const { return const_cast<TMap*>(this)->Find(Key); }

	friend FORCEINLINE FArchive& operator<<(FArchive& Ar, TMap& Map) { return Ar << (TArray<TMapPair<TK, TV>>&)Map; }
};
...
class FString
{
  public:
	...
	// rvalue functions
	FString(FString&& Other) : Data(MoveTemp(Other.Data)) {}
	...
	FORCEINLINE FString& operator+=(const FString& Str) { return operator+=(*Str); }

	// use FString as allocated char*, FString became empty and will not free
	// detached string in destructor
	char* Detach();

	FORCEINLINE int Len() const { return Data.Num() <= 1 ? 0 : Data.Num() - 1; }

	FORCEINLINE TArray<char>& GetDataArray() { return Data; }

	FORCEINLINE const TArray<char>& GetDataArray() const { return Data; }

	FORCEINLINE void Empty(int count = 0) { Data.Empty(count); }
	...
	FORCEINLINE void RemoveAt(int index, int count = 1) { Data.RemoveAt(index, count); }

	char& operator[](int index)
	{
		if (unsigned(index >= Data.Num())) appError("FString[%d/%d]", index, Data.Num());
	...
	FORCEINLINE const char* operator*() const { return IsEmpty() ? "" : Data.GetData(); }
	//	FORCEINLINE operator const char*() const
	//	{
	//		return IsEmpty() ? "" : Data.GetData();
	//	}
	// comparison
	friend FORCEINLINE bool operator==(const FString& A, const FString& B) { return strcmp(*A, *B) == 0; }
	friend FORCEINLINE bool operator==(const char* A, const FString& B) { return strcmp(A, *B) == 0; }
	friend FORCEINLINE bool operator==(const FString& A, const char* B) { return strcmp(*A, B) == 0; }
	friend FORCEINLINE bool operator!=(const FString& A, const FString& B) { return strcmp(*A, *B) != 0; }
	friend FORCEINLINE bool operator!=(const char* A, const FString& B) { return strcmp(A, *B) != 0; }
	friend FORCEINLINE bool operator!=(const FString& A, const char* B) { return strcmp(*A, B) != 0; }
...
class FStaticString : public FString
{
  public:
	...
	// operators
	FORCEINLINE FStaticString& operator=(const char* src) { return (FStaticString&)FString::operator=(src); }
	FORCEINLINE FStaticString& operator=(const FString& src) { return (FStaticString&)FString::operator=(src); }
...
struct FColor
{
	...
	FColor() {}
	FColor(byte r, byte g, byte b, byte a = 255) : R(r), G(g), B(b), A(a) {}
	friend FArchive& operator<<(FArchive& Ar, FColor& C)
	{
		if (Ar.Game < GAME_UE3) return Ar << C.R << C.G << C.B << C.A;
...
class FGuid
{
  public:
	...
	FORCEINLINE bool operator==(const FGuid& Other) const { return memcmp(this, &Other, sizeof(FGuid)) == 0; }
};
...
#define BULKDATA_PayloadAtEndOfFile                                                                                    \
	0x00001 // bulk data stored at the end of this file, data offset added to global data offset in package
...
#define BULKDATA_OptionalPayload                                                                                       \
	0x00800						   // same as BULKDATA_PayloadInSeperateFile, but stored with .uptnl extension (UE4.20+)
...
struct FByteBulkData //?? separate FUntypedBulkData
{
	...
	FByteBulkData()
		: BulkData(NULL), BulkDataOffsetInFile(0)
#if UNREAL4
		  ,
		  bIsUE4Data(false)
#endif
	...
	virtual ~FByteBulkData() { ReleaseData(); }
...
int appDecompress(byte* CompressedBuffer, int CompressedSize, byte* UncompressedBuffer, int UncompressedSize,
				  int Flags);
...
class FStripDataFlags
{
  public:
	...
	FORCEINLINE bool IsEditorDataStripped() const { return (GlobalStripFlags & 1) != 0; }

	FORCEINLINE bool IsDataStrippedForServer() const { return (GlobalStripFlags & 2) != 0; }

	FORCEINLINE bool IsClassDataStripped(byte Flag) const { return (ClassStripFlags & Flag) != 0; }
