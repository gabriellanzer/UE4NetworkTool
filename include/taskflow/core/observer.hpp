...
inline void TFProfObserver::Summary::dump_tsum(std::ostream& os) const {
  ...
  std::for_each(tsum.begin(), tsum.end(), [&](const auto& i){
    if(i.count == 0) return;
    count_w = max(count_w, std::to_string(i.count).size());
  });
  
  std::for_each(tsum.begin(), tsum.end(), [&](const auto& i){
    if(i.count == 0) return;
    time_w = max(time_w, std::to_string(i.total_span).size());
  });
  
  std::for_each(tsum.begin(), tsum.end(), [&](const auto& i){
    if(i.count == 0) return;
    avg_w = max(time_w, std::to_string(i.avg_span()).size());
  });
  
  std::for_each(tsum.begin(), tsum.end(), [&](const auto& i){
    if(i.count == 0) return;
    min_w = max(min_w, std::to_string(i.min_span).size());
  });
  
  std::for_each(tsum.begin(), tsum.end(), [&](const auto& i){
    if(i.count == 0) return;
    max_w = max(max_w, std::to_string(i.max_span).size());
  });
...
inline void TFProfObserver::Summary::dump_wsum(std::ostream& os) const {
  ...
  std::for_each(wsum.begin(), wsum.end(), [&](const auto& i){
    if(i.count == 0) return;
    l_w = max(l_w, std::to_string(i.level).size());
  });
  
  std::for_each(wsum.begin(), wsum.end(), [&](const auto& i){
    if(i.count == 0) return;
    c_w = max(c_w, std::to_string(i.count).size());
  });
  
  std::for_each(wsum.begin(), wsum.end(), [&](const auto& i){
    if(i.count == 0) return;
    d_w = max(d_w, std::to_string(i.total_span).size());
  });
  
  std::for_each(wsum.begin(), wsum.end(), [&](const auto& i){
    if(i.count == 0) return;
    avg_w = max(avg_w, std::to_string(i.avg_span()).size());
  });
  ...
  for(size_t w=first; w<_timeline.segments.size(); w++) {
    for(size_t l=0; l<_timeline.segments[w].size(); l++) {
      ...
      for(size_t i=0; i<_timeline.segments[w][l].size(); ++i) {
        ...
        view_beg = view_beg ? min(*view_beg, s.beg) : s.beg;
        view_end = view_end ? max(*view_end, s.end) : s.end;
        ...
        x.count += 1;
        x.total_span += t;
        x.min_span = (x.count == 1) ? t : min(t, x.min_span);
        x.max_span = (x.count == 1) ? t : max(t, x.max_span);

        // update the worker summary
        ws.total_span += t;
        ws.min_span = (i == 0) ? t : min(t, ws.min_span);
        ws.max_span = (i == 0) ? t : max(t, ws.max_span);

        auto&y = ws.tsum[static_cast<int>(s.type)];
        y.count += 1;
        y.total_span += t;
        y.min_span = (y.count == 1) ? t : min(t, y.min_span);
        y.max_span = (y.count == 1) ? t : max(t, y.max_span);
        ...
        //  ).count();
        //  ws.total_delay += d;
        //  ws.min_delay = (i == 1) ? d : min(ws.min_delay, d);
        //  ws.max_delay = (i == 1) ? d : max(ws.max_delay, d);
        //}
      }
